      ...
      try {
        ...
        const formattedCommits = fetchedCommits
          ...
          .map((commit) => ({
            ...
            messages: commit.payload.commits.map((commit) => commit.message),
          }));
    ...
    <Card className="relative pb-5 sm:px-4 pt-7 max-w-7xl">
      ...
      {commits.error && (
        <div className="flex flex-col items-center max-w-xl">
          <img src={NotFoundAnimate} alt="" className="lg:max-w-56 max-w-36" />
          <span className="px-8 pt-2 text-center font-robotoMono">
            The GitHub server could not be joined or an error happened while
            fetching the data
          </span>
        </div>
      )}
